...
class SAPE_client extends SAPE_base
{
    ...
    var $_return_links_calls;
    var $_teasers_css_showed = false;
    ...
    function _add_debug_info($html)
    {
        if ($this->_debug) {
            $html .= $this->_debug_output($this->_get_full_user_agent_string());
    ...
    function _add_debug_info_teasers($html)
    {
        $html = $this->_add_debug_info($html);

        if ($this->_debug) {
            $images_path_contents = 'No __sape_teaser_images_path__ in links.db';

            if (isset($this->_links['__sape_teaser_images_path__'])) {

                if ('' == $this->_links['__sape_teaser_images_path__']) {
                    $images_path_contents = 'Path __sape_teaser_images_path__ is empty';
                }
                else {

                    $images_file_path = realpath(
                        rtrim($_SERVER['DOCUMENT_ROOT'], DIRECTORY_SEPARATOR)
                        . DIRECTORY_SEPARATOR
                        . strtok($this->_links['__sape_teaser_images_path__'], '?')
                    );

                    $images_path_contents = @file_get_contents($images_file_path);
                    if (false === $images_path_contents) {
                        $images_path_contents = 'Cannot get file contents';
                    }

                    $images_path_contents = array(
                        'path'     => $images_file_path,
                        'contents' => $images_path_contents
                    );
                }
            }

            $html .= $this->_debug_output($images_path_contents);
        }
    ...
    function return_teasers_block($block_id)
    {
        if ($this->_debug) {
            if (function_exists('debug_backtrace')) {
                $this->_return_links_calls[] = debug_backtrace();
        ...
        if (count($this->_teasers_page) && false==empty($template)) {
            ...
            $this->raise_error("Нет информации по блоку $block_id, обратитесь в службу поддержки");
        }
        ...
        return $this->_add_debug_info_teasers($this->_return_obligatory_page_content() . $html);
    }

    function show_image($file_name = null)
    {
        if ($this->_debug) {
            if (function_exists('debug_backtrace')) {
                $this->_return_links_calls[] = debug_backtrace();
        ...
        if (!array_key_exists('__sape_teaser_images__', $this->_links) || !array_key_exists($file_name, $this->_links['__sape_teaser_images__'])) {
            $this->raise_error('Нет файла изображения с именем ' . $file_name);
